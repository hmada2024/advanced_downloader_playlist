...
# Conditional import for type hinting to avoid circular imports
if TYPE_CHECKING:
    from ..logic.history_manager import HistoryManager
    from .interface import UserInterface # Assuming interface.py holds UserInterface
...
MAX_TITLE_DISPLAY_LEN = 60 # Max length for title/URL display in entry
...
class HistoryTab(ctk.CTkFrame):
    ...
    def __init__(self, master: Any, history_manager: 'HistoryManager', ui_interface_ref: 'UserInterface', **kwargs: Any):
        """
        ...
        self.scrollable_frame.grid(row=0, column=0, padx=10, pady=(10, 5), sticky="nsew")
        self.scrollable_frame.grid_columnconfigure(0, weight=1) # Allow entry frames to expand horizontally
        ...
        # --- Initial Load ---
        self.load_history() # Load entries when the tab is created
        print("HistoryTab: Initialization complete.")


    def load_history(self) -> None:
        """Loads history entries from the manager and displays them."""
        print("HistoryTab: Loading history...")
        # 1. Clear existing widgets in the scrollable frame
        for widget in self.scrollable_frame.winfo_children():
            widget.destroy()

        # 2. Get entries from the database
        entries: List[Dict[str, Any]] = self.history_manager.get_all_entries()

        # 3. Check if history is empty
        if not entries:
            no_history_label = ctk.CTkLabel(self.scrollable_frame, text=NO_HISTORY_MSG, text_color="gray")
            no_history_label.pack(pady=20)
            self.clear_button.configure(state="disabled") # Disable clear button if no history
            print("HistoryTab: No history entries found.")
            return
        else:
             self.clear_button.configure(state="normal") # Enable clear button if history exists


        # 4. Create and display widgets for each entry
        for entry_data in entries:
            entry_frame = ctk.CTkFrame(self.scrollable_frame, fg_color="gray14") # Slightly different background
            entry_frame.pack(fill="x", padx=5, pady=(0, 5)) # Use pack within scrollable frame
            ...
            details_text = f"{entry_data['timestamp']}  |  Type: {entry_data['operation_type']}"
            details_label = ctk.CTkLabel(info_frame, text=details_text, anchor="w", text_color="gray", font=ctk.CTkFont(size=11))
            details_label.pack(fill="x")

            # --- Right side: Action Buttons ---
            button_frame = ctk.CTkFrame(entry_frame, fg_color="transparent")
            button_frame.grid(row=0, column=1, padx=5, pady=5, sticky="e")

            use_button = ctk.CTkButton(
                button_frame,
                text=BTN_USE_AGAIN,
                width=80,
                # Use lambda to capture current entry_data for the command
    ...
    def _handle_use_again(self, entry_data: Dict[str, Any]) -> None:
        ...
        if op_type in {'Download', 'Fetch Info'}:
            ...
            print(f"HistoryTab Warning: Unknown operation type '{op_type}' for Use Again.")
            messagebox.showwarning("Action Error", f"Cannot automatically reuse entry with type: {op_type}")
    ...
    def _handle_copy(self, url: str) -> None:
        ...
        try:
            ...
            # Update main status bar via ui_interface reference
            self.ui_interface.update_status("URL copied to clipboard!")
            # Optional: Reset status after a delay
            self.after(3000, lambda: self.ui_interface.update_status("Ready.")) # Use main status for consistency
    ...
    def _handle_delete(self, entry_id: int) -> None:
        ...
        if messagebox.askyesno(CONFIRM_DELETE_TITLE, CONFIRM_DELETE_MSG):
            if self.history_manager.delete_entry(entry_id):
                ...
                print(f"HistoryTab Error: Failed to delete entry {entry_id} from database.")
                messagebox.showerror("Database Error", "Could not delete the history entry.")
        else:
    ...
    def _handle_clear_history(self) -> None:
        ...
        if messagebox.askyesno(CONFIRM_CLEAR_TITLE, CONFIRM_CLEAR_MSG):
            if self.history_manager.clear_all_entries():
                ...
                self.load_history() # Refresh the display (will show "No history")
